...
export class FileuploadService {
  ...
  async create(createFileuploadDto: CreateFileuploadDto) : Promise<FileUpload> {
    ...
    //let bugReport = await this.bugreportService.findOne(createFileuploadDto.bugReportId);
    ...
    //fileUpload.bugReportId = bugReport;
  ...
  findOne(id: number) :Promise<FileUpload> {
    return this.FileUploadRepository.findOneBy({id : id});
  }

  async update(id: number, updateFileuploadDto: UpdateFileuploadDto) {
    let fileUpload = new FileUpload();
    //let bugReport = await this.bugreportService.findOne(updateFileuploadDto.bugReportId);
    fileUpload.id = id;
